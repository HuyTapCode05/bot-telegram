...
// Minimal direct conversion of the Zalo handler to a Telegram command
function delay(ms) { return new Promise(resolve => setTimeout(resolve, ms)); }

// small HTML escape helper for safety when using parse_mode=HTML
...
function stripHtmlTags(input) {
  if (!input) return '';
  // remove tags like <...> and return inner text-ish
  let s = String(input);
  // remove tags
  s = s.replace(/<[^>]*>/g, '');
  // remove attribute-like tokens name=... id=... etc
  s = s.replace(/\b\w+\s*=\s*"[^"]*"/g, '');
  s = s.replace(/\b\w+\s*=\s*'[^']*'/g, '');
  // remove leftover equals/quotes/extra punctuation
  s = s.replace(/["'=<>\/]/g, '');
  // collapse whitespace
...
module.exports = async function phatnguoi(ctx) {
  ...
  // Quick validation: many VN plates have a numeric tail of 4-5 digits.
  // If the numeric tail is missing or shorter than 4, the API often replies "Chưa hỗ trợ định dạng biển số này." —
  // so we ask the user to provide the full plate instead of calling the API.
  try {
    ...
    // fallthrough and continue if something unexpected
  ...
  // API: status 1 = ok with data array, status 2 = ok but no data, status 3 = unsupported format
  if (data && data.status === 1 && Array.isArray(data.data) && data.data.length > 0) {
      ...
      // status 2: API indicates no violations found — format reply like the screenshot
        // If user provided a vehicle type argument, prefer that
        let vehicleType = null;
        if (userVehicleArg) {
          const v = String(userVehicleArg || '').toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '');
          // map common inputs to readable labels
          const map = {
            'xemay': 'Xe máy', 'xemáy': 'Xe máy', 'moto': 'Xe máy', 'môto': 'Xe máy', 'moto': 'Xe máy',
            'oto': 'Ô tô', 'ôto': 'Ô tô', 'xeoto': 'Ô tô', 'xetai': 'Xe tải', 'xetải': 'Xe tải'
          };
          vehicleType = map[v] || (userVehicleArg ? userVehicleArg : null);
        }
      try {
        const helper = require('../../tienich/checkphatnguoi');
  ...
  if (!vehicleType && site && site.ok && site.vehicleType) vehicleType = site.vehicleType;
      ...
      // sanitize vehicleType from any HTML the helper might return
