...
async function zingmp3Command(ctx) {
  ...
  try {
    ...
    let items = [];
    if (res.data && Array.isArray(res.data.items)) items = res.data.items.slice(0, 8);
    else if (Array.isArray(res.data)) items = res.data.slice(0, 8);
    ...
    const sent = await ctx.reply(reply, { parse_mode: 'HTML' });
    ...
    musicSelectionsMapZing.set(msgIdKey, store);
    if (chatKey) musicSelectionsMapZing.set(chatKey, store);
    // If user provided a trailing number in the original command (/zingmp3 keyword 2), play immediately
    ...
    if (immediateNumber && store.collection && store.collection.length >= immediateNumber) {
      await sendSelectionAudioZing(ctx, immediateNumber - 1, msgIdKey);
    }
    return;
  } catch (err) {
